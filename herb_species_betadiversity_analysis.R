...
## Creation: July 06, 2016                                                    ## 
##                                                                            ## 
## Discription: Null model analysis to determine how the processes driving    ##
##              the processes driving beta diversity patterns change along    ##
...
# You need to have in the working directory files with spatial eigenvectors 
# ("SpatialEigenVectors_Unburned.txt") and spatial eigenvalues 
# (SpatialEigenValues_Unburned.txt), environmental characteristics of the plots 
# (either "Env_unburned.txt" for analysis based on a substet of variables or 
# "Env_Unburned_PCA.txt" for analysis based on PCA), a file with the geographic
# coordinates of the plots ("OzarkFire_Plots_20121129.txt"), and a file in which 
# each row is a different individual and two of the columns are plot code and 
# species code ("OzarkFire_Plants_20121204.txt").
#-------------------------------------------------------------------------------
...
dist.centroid.ses.low.med <- as.vector(c(betadisp.ses.low$distance, betadisp.ses.med$distance))
dist.centroid.ses.low.high <- as.vector(c(betadisp.ses.low$distance, betadisp.ses.high$distance))
dist.centroid.ses.med.high <- as.vector(c(betadisp.ses.med$distance, betadisp.ses.high$distance))

prod.low.med <- c(rep("low", length(betadisp.bcLow$distance)), rep("med", length(betadisp.bcMed$distance)))
prod.low.high <- c(rep("low", length(betadisp.bcLow$distance)), rep("high", length(betadisp.bcHigh$distance)))
prod.med.high <- c(rep("med", length(betadisp.bcMed$distance)), rep("high", length(betadisp.bcHigh$distance)))
...
for (i in 1:k){
  ...
  null.F.ses.low.med <- append(null.F.ses.low.med, anova(permANOVA.ses.low.med)$"F value"[1], 
                               after = length(null.F.ses.low.med)) 
}

# Plot results
hist(c(null.F.ses.low.med, obs.F.ses.low.med), main = "F permutation test, 95 percentile = dotted line, seserved = red line", xlab="F", cex.axis=1.5, cex.lab=1.5)
...
for (i in 1:k){
  ...
  null.F.ses.low.high <- append(null.F.ses.low.high, anova(permANOVA.ses.low.high)$"F value"[1], 
                                after = length(null.F.ses.low.high)) 
}

# Plot results
hist(c(null.F.ses.low.high, obs.F.ses.low.high), main = "F permutation test, 95 percentile = dotted line, seserved = red line", xlab="F", cex.axis=1.5, cex.lab=1.5)
...
for (i in 1:k){
  ...
  null.F.ses.med.high <- append(null.F.ses.med.high, anova(permANOVA.ses.med.high)$"F value"[1], 
                                after = length(null.F.ses.med.high)) 
}

# Plot results
hist(c(null.F.ses.med.high, obs.F.ses.med.high), main = "F permutation test, 95 percentile = dotted line, seserved = red line", xlab="F", cex.axis=1.5, cex.lab=1.5)
